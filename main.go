...
func main() {
	var (
		...
		disableExporterMetrics = app.Flag("web.disable-exporter-metrics", "Exclude metrics about the exporter itself (promhttp_*, process_*, go_*).").Default("false").Bool()
	...
	*routePrefix = computeRoutePrefix(*routePrefix, *externalURL)
	...
	if *disableExporterMetrics {
		gr := prometheus.NewRegistry()
		gr.MustRegister(versioncollector.NewCollector("pushgateway"))
		gatherer = gr
	}

	ms := storage.NewDiskMetricStore(*persistenceFile, *persistenceInterval, gatherer, logger)

	// Create a Gatherer combining the DefaultGatherer or empty and the metrics from the metric store.
